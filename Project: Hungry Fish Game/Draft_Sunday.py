...
bg_music = pygame.mixer.Sound('non_copyright_water_music.mp3') 
...
def instructions():
    ...
    level1_instructions = Text(Point(333, 100), f'Catch {LEVEL_UP} yellow fishes to upgrade to level 2.')
    level1_instructions.setSize(15)
    level1_instructions.draw(instructions_window)
    
    level2_instructions = Text(Point(333, 120), f'Catch {LEVEL_UP} sharks to upgrade to level 3. ')
    level2_instructions.setSize(15)
    level2_instructions.draw(instructions_window)
    
    level3_instructions = Text(Point(333, 140), f'Catch {LEVEL_UP} orcas to win the game. ')
    level3_instructions.setSize(15)
    level3_instructions.draw(instructions_window)
    
    eat_instructions = Text(Point(333, 200), 'If you are not in level 2, you can not eat the shark. ')
    eat_instructions.setSize(15)
    eat_instructions.draw(instructions_window)
    
    lose_instructions = Text(Point(333, 220), 'Attempt to eat the shark will make you lose the game. ')
...
def game_loop(window, fish):
    ...
    while orca_caught < LEVEL_UP and hasNOTLost:
        ...
        if random.randrange(100) < 6: #% of a time I will get an orca  
